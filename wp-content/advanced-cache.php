...
// WP-Optimize advanced-cache.php (written by version: 3.5.0) (homeurl: https://solance.in/) (abspath: /var/www/html/) (do not change this line, it is used for correctness checks)
...
if (false === $plugin_location) {
	...
	if (strcasecmp('https://solance.in/', $protocol . '://' . $host . $request_uri) === 0) {
...
if (!@file_exists(WPO_CACHE_CONFIG_DIR . '/config-solance.in.php')) { return; }

$GLOBALS['wpo_cache_config'] = @json_decode(file_get_contents(WPO_CACHE_CONFIG_DIR . '/config-solance.in.php'), true);

if (empty($GLOBALS['wpo_cache_config'])) {
	include_once(WPO_CACHE_CONFIG_DIR . '/config-solance.in.php');
}
